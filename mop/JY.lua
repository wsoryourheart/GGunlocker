...
textout(Check_UI("监狱 - "..Function_Version,"Stockade - "..Function_Version))
...
local Dungeon_In = {mapid = 1453, x = -8758, y = 850, z = 80}
local Dungeon_Out = {mapid = 1453, x = 40, y = 0, z = -15}
local Dungeon_Flush_Point = {mapid = 1453, x = -8773, y = 840, z = 91}
...
local Arrive_Once = false -- 到达一次

local Forst = false -- 冰霜新星
local Bx,By,Bz = 0,0,0 -- 冰霜新星位置
...
function JY()
	...
	if Dungeon_step1 == 2 then -- 1
		...
		local Path = 
		{
		{83.2619, 0.4925, -25.6062},
		{84.4299, -2.9165, -26.0476},
		{84.4736, -28.9262, -26.5229},
		{84.8010, -7.3716, -26.5229},
		{84.8542, 3.8283, -25.8169},
		{84.9972, 29.7277, -26.5273},
		{84.4245, 17.0713, -26.5240},
		}
		...
		if Dungeon_move == 7 then
			...
			for i = 1,#Monster do
				...
				if CheckDebuffByName(Monster[i],rs["冰霜新星"]) and Spell_Castable(rs["烈焰风暴"]) and not CastingBarFrame:IsVisible() and GetTime() - JY_Time >= 8 and UnitPower('player') >= 3000 then
					JY_Timer = false
					...
					if awm.IsAoEPending() then
						awm.ClickPosition(Bx,By,Bz)
					end
					return
				elseif CastingBarFrame:IsVisible() then
					if not JY_Timer then
						JY_Timer = true
						JY_Time = GetTime()
					end
					return
				...
				if awm.UnitHealth(Monster[i]) <= 2000 and C_distance <= 10 and Spell_Castable(rs["冰霜新星"]) then
					...
					Forst = true
					Bx,By,Bz = awm.ObjectPosition("player")
	...
	if Dungeon_step1 == 5 then -- 2
		...
		local Path = 
		{
		{105.7903, 0.7320, -25.6064},
		{106.6462, -3.0930, -26.1179},
		{106.6058, -28.3469, -26.5229},
		{106.7900, -8.2629, -26.5229},
		{106.7435, 2.6564, -25.6062},
		{106.9463, 30.0251, -26.5229},
		{106.9595, 15.7670, -26.5229},
		{106.9945, 9.9674, -26.5229},
		{106.9169, 1.7680, -25.6072},
		}
		if Dungeon_move > #Path then
		    Dungeon_step1 = Dungeon_step1 + 1
			Dungeon_move = 1
	...
	if Dungeon_step1 == 8 then -- 3
	    Note_Head = Check_UI("拉怪","Pulling mobs")..Dungeon_step1

		local Path = 
		{
		{128.8342, 0.1829, -25.6072},
		{142.7572, 0.6035, -25.6072},
		{163.5428, 0.3156, -25.6072},
		{154.6700, 0.7274, -25.6072},
		{149.6700, 0.7274, -25.6072},
		{148.7317, 0.8629, -25.6072},
		{130.1951, 0.4543, -25.6072},
		}
		if Dungeon_move > #Path then
		    Dungeon_step1 = Dungeon_step1 + 1
			Dungeon_move = 1
	...
	if Dungeon_step1 == 11 then -- 4
	    Note_Head = Check_UI("拉怪","Pulling mobs")..Dungeon_step1

		local Path = 
		{
		{127.9053, -42.4525, -33.9395},
		{125.2979, -42.9765, -34.0768},
		{99.7011, -40.1920, -34.8562},
		{118.2524, -42.9750, -34.8562},
		{126.9080, -43.5813, -33.9407},
		{155.3283, -47.0794, -34.8562},
		{143.4034, -45.4302, -34.8562},
		{137.0677, -44.6446, -34.8562},
		{129.2367, -43.6769, -33.9396},
		}
		if Dungeon_move > #Path then
		    Dungeon_step1 = Dungeon_step1 + 1
			Dungeon_move = 1
	...
	if Dungeon_step1 == 14 then -- 5
	    Note_Head = Check_UI("拉怪","Pulling mobs")..Dungeon_step1

		local Path = 
		{
		{123.5047, -64.0227, -33.9396},
		{120.0096, -63.8467, -34.4880},
		{97.8467, -57.6495, -34.8562},
		{113.2936, -62.8848, -34.8562},
		{126.3584, -65.9201, -34.0149},
		{140.8201, -70.3070, -34.8562},
		{151.3741, -73.5997, -34.8562},
		{154.4178, -66.1768, -34.8562},
		{151.7473, -74.1803, -34.8562},
		{139.0962, -69.7799, -34.8562},
		{133.9642, -68.7569, -34.8562},
		}
		if Dungeon_move > #Path then
		    Dungeon_step1 = Dungeon_step1 + 1
			Dungeon_move = 1
	...
	if Dungeon_step1 == 17 then -- 6
	    Note_Head = Check_UI("拉怪","Pulling mobs")..Dungeon_step1

		local Path = 
		{
		{114.1202, -89.0475, -33.9404},
		{105.2334, -105.5669, -35.1895},
		{92.5161, -99.3855, -34.3866},
		{69.8955, -87.2183, -33.9395},
		{80.5237, -93.1364, -33.9395},
		{85.9408, -96.1724, -33.9395},
		{102.1004, -104.3121, -35.1895},
		}
		if Dungeon_move > #Path then
		    Dungeon_step1 = Dungeon_step1 + 1
			Dungeon_move = 1
	...
	if Dungeon_step1 == 20 then -- 7
	    Note_Head = Check_UI("拉怪","Pulling mobs")..Dungeon_step1

		local Path = 
		{
		{117.9733, -113.0636, -34.2212},
		{138.9151, -124.1979, -33.9398},
		{124.8845, -115.6166, -33.9398},
		{113.7330, -110.9874, -35.1896},
		{100.2818, -116.5109, -35.1896},
		{97.9990, -119.3423, -34.0319},
		{85.9198, -141.6910, -33.9395},
		{92.5330, -130.5607, -33.9395},
		{95.6145, -124.7963, -33.9395},
		{104.5282, -107.3667, -35.1895},
		}
		if Dungeon_move > #Path then
		    Dungeon_step1 = Dungeon_step1 + 1
			Dungeon_move = 1
	...
	if Dungeon_step1 == 23 then -- 8
	    Note_Head = Check_UI("拉怪","Pulling mobs")..Dungeon_step1

		local Path = 
		{
		{111.5757, -93.9897, -34.6447},
		{121.8592, -71.6535, -33.9396},
		{126.1496, -54.5774, -33.9396},
		{129.6319, 0.9516, -25.6062},
		{130.3851, 45.5677, -33.9391},
		{134.3067, 44.8321, -34.6079},
		{159.4709, 41.3826, -34.8562},
		{139.9732, 44.4590, -34.8562},
		{126.4502, 45.6129, -34.3806},
		{100.9613, 48.7847, -34.8562},
		{113.1008, 47.4670, -34.8562},
		{119.5666, 46.3294, -34.8562},
		{129.7087, 45.2195, -33.9396},
		}
		if Dungeon_move > #Path then
		    Dungeon_step1 = Dungeon_step1 + 1
			Dungeon_move = 1
	...
	if Dungeon_step1 == 26 then -- 9
	    Note_Head = Check_UI("拉怪","Pulling mobs")..Dungeon_step1

		local Path = 
		{
		{134.4138, 66.1881, -33.9396},
		{138.2775, 65.5261, -34.4957},
		{163.3386, 58.4912, -34.8562},
		{143.2912, 64.7040, -34.8562},
		{131.3619, 67.9809, -34.2675},
		{107.0853, 74.9153, -34.8562},
		{104.1148, 69.5318, -34.8562},
		{105.8953, 76.9722, -34.8562},
		{120.6960, 70.8358, -34.8562},
		{125.6960, 70.8358, -34.8562},
		{133.9945, 66.9313, -33.9396},
		}
		if Dungeon_move > #Path then
		    Dungeon_step1 = Dungeon_step1 + 1
			Dungeon_move = 1
	...
	if Dungeon_step1 == 29 then -- 10
	    Note_Head = Check_UI("拉怪","Pulling mobs")..Dungeon_step1

		local Path = 
		{
		{147.0124, 88.5144, -33.9395},
		{143.2383, 88.4749, -33.9396},
		{153.4426, 107.8421, -35.1896},
		{166.2399, 100.7823, -34.1239},
		{188.8357, 88.8889, -33.9395},
		{177.0691, 95.3503, -33.9395},
		{172.3469, 97.4314, -33.9395},
		{156.5563, 105.8053, -35.1896},
		}
		if Dungeon_move > #Path then
		    Dungeon_step1 = Dungeon_step1 + 1
			Dungeon_move = 1
	...
	if Dungeon_step1 == 32 then -- 11
	    Note_Head = Check_UI("拉怪","Pulling mobs")..Dungeon_step1

		local Path = 
		{
		{140.9324, 113.8352, -34.6961},
		{118.0528, 126.2597, -33.9395},
		{135.6307, 116.7242, -33.9395},
		{144.2908, 112.4734, -35.1896},
		{158.8421, 119.0062, -35.1896},
		{172.4565, 143.4461, -33.9395},
		{164.6866, 129.7285, -33.9396},
		{164.6866, 124.7285, -33.9396},
		{155.3938, 112.0767, -35.1895},
		}
		if Dungeon_move > #Path then
		    Dungeon_step1 = Dungeon_step1 + 1
			Dungeon_move = 1
	...
	if Dungeon_step1 == 35 then -- 12
	    Note_Head = Check_UI("拉怪","Pulling mobs")..Dungeon_step1

		local Path = 
		{
		{147.5740, 96.8941, -35.1895},
		{142.2500, 88.0911, -33.9396},
		{134.7318, 68.7198, -33.9396},
		{129.3196, 35.8490, -33.9396},
		{128.9744, 0.7690, -25.6062},
		{107.9228, 0.8519, -25.6062},
		{72.2329, 1.4110, -25.1173},
		}
		if Dungeon_move > #Path then
		    Dungeon_step1 = Dungeon_step1 + 1
			Dungeon_move = 1
...
function MainThread()
	...
	if (Easy_Data["需要卖物"] or Easy_Data["需要修理"]) then
	    if not Check_BagFree() or Sell.Step ~= 1 then
			if Easy_Data["自定义商人"] then
				...
				Merchant_Coord.mapid, Merchant_Coord.x,Merchant_Coord.y,Merchant_Coord.z = 1453, -8787, 609,96
				Merchant_Name = 1287
			end

			if Instance == 34 and Dungeon_step == 2 then
		    ...
		    elseif Instance ~= 34 then
				...
				if GetItemCount(6948) > 0 and durationtime < 10 then
				    ...
				    if not Has_Mail and Easy_Data["需要邮寄"] and Easy_Data["卖物前邮寄"] then
						Mail_Coord.mapid,Mail_Coord.x,Mail_Coord.y,Mail_Coord.z = 1453, -8876,650,96
	...
	if Instance == 34 then
		...
		if Dungeon_Enter_step == 1 then
		    mapid,x,y,z = Dungeon_In.mapid,-8767.74,850,87.29
		elseif Dungeon_Enter_step == 2 then
		    mapid,x,y,z = Dungeon_In.mapid,-8767.74,850,87.29
		elseif Dungeon_Enter_step == 3 then
		    mapid,x,y,z = Dungeon_In.mapid,-8766.27,849.26,87.16
		end
		...
		if distance > 1 and not Dungeon_Flush then
			...
			if (Easy_Data["服务器地图"] and Current_Map ~= 1453 and PlayerFrame:IsVisible()) or Easy_Data.Sever_Map_Calculated or Continent_Move then
				Note_Set(Check_UI("使用云地图包进行寻路","Using Sever Map System"))
				Sever_Run(Current_Map,1453,Dungeon_In.x,Dungeon_In.y,Dungeon_In.z)
				return
			end
		...
		elseif distance <= 1 and not Dungeon_Flush then
		    if Dungeon_Enter_step == 2 then
			    C_Timer.After(2,function() if Dungeon_Enter_step == 2 then Dungeon_Enter_step = 3 end end)
			else
...
local function Create_Config_UI() -- 游戏设置
	...
	local function Wait_point()
	    Basic_UI.Set.Py = Basic_UI.Set.Py - 30
	    local Header1 = Create_Header(Basic_UI.Set.frame,"TOPLEFT",10, Basic_UI.Set.Py,Check_UI("监狱 爆本 本外等待坐标","Stockade Wait Point")) 

		Basic_UI.Set.Py = Basic_UI.Set.Py - 20
		Basic_UI.Set["监狱等待坐标"] = Create_EditBox(Basic_UI.Set.frame,"TOPLEFT",10, Basic_UI.Set.Py,"-8773,840,91",false,280,24)
		Basic_UI.Set["监狱等待坐标"]:SetScript("OnEditFocusLost", function(self)
			Easy_Data["监狱等待坐标"] = Basic_UI.Set["监狱等待坐标"]:GetText()
			local coord_package = string.split(Easy_Data["监狱等待坐标"],",")
		...
		if Easy_Data["监狱等待坐标"] ~= nil then
			Basic_UI.Set["监狱等待坐标"]:SetText(Easy_Data["监狱等待坐标"])
			local coord_package = string.split(Easy_Data["监狱等待坐标"],",")
			Dungeon_Flush_Point.x,Dungeon_Flush_Point.y,Dungeon_Flush_Point.z = tonumber(coord_package[1]),tonumber(coord_package[2]),tonumber(coord_package[3])
		else
			Easy_Data["监狱等待坐标"] = Basic_UI.Set["监狱等待坐标"]:GetText()
		...
		Basic_UI.Set["获取等待坐标"]:SetScript("OnClick", function(self)
			...
			if Instance ~= 34 then
				...
				Basic_UI.Set["监狱等待坐标"]:SetText(math.floor(x)..","..math.floor(y)..","..math.floor(z))
				Easy_Data["监狱等待坐标"] = Basic_UI.Set["监狱等待坐标"]:GetText()
