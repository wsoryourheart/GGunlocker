...
local Merchant_Coord = {mapid = 1415, x = -1707, y = -1424, z = 34}
local Merchant_Name = "匠人比尔"

local Mail_Coord = {mapid = 1415, x = -1656, y = -1344, z = 32}
...
function XS()
	...
	if Dungeon_step1 == 5 and CheckDebuffByName("player",Check_Client("减速术","Slow")) and Spell_Castable(rs["寒冰屏障"]) and Spell_Castable(rs["急速冷却"]) and Dungeon_move <= 30 then
	    awm.CastSpellByName(rs["寒冰屏障"])
	end

	if CheckBuff("player",rs["寒冰屏障"]) and Dungeon_move <= 30 and Dungeon_step1 == 5 then
    ...
    if Dungeon_step1 == 1 then
		...
		local x,y,z = 871.19,1322.86,18.00
		...
		if distance > 1 then
			...
			if Spell_Castable(rs["魔爆术(等级 1)"]) then
	...
	if Dungeon_step1 == 2 then -- 击杀门口怪物
	    Note_Head = Check_UI("击杀门口怪物","Kill Door Mob")

		local target = nil

		if not UnitAffectingCombat("player") then
		    target = Find_Object_Position(nil,865,1322,18,40)
			if target ~= nil and not awm.UnitIsDead(target) and awm.UnitCanAttack("player",target) then
			    Note_Set(Check_UI("吸引怪物 = ","Attract mob = ")..awm.UnitFullName(target))
			    awm.TargetUnit(target)
				local tarx,tary,tarz = awm.ObjectPosition(target)

				local flags = bit.bor(0x10, 0x100, 0x1)
				local hit = awm.TraceLine(Px,Py,Pz+2.25,tarx,tary,tarz+2.25,flags)
				if hit == 1 then
					Run(tarx,tary,tarz)
					return
				end

				if awm.GetDistanceBetweenObjects("player",target) > 20 then
				    Run(tarx,tary,tarz)
					return
				end
				if Spell_Castable(rs["火焰冲击"]) then
	...
	if Dungeon_step1 == 3 then -- 回位
	    local x,y,z = 870,1322,18
	    local distance = awm.GetDistanceBetweenPositions(Px,Py,Pz,870,1322,18)
		Note_Head = Check_UI("回位","Back to position")
	...
	if Dungeon_step1 == 4 then -- 血蓝恢复
		...
		if not awm.UnitAffectingCombat("player") then
			...
			local starttime, duration, enabled, _ = GetSpellCooldown(rs["寒冰屏障"])
			local endtime = starttime + duration
			if endtime > GetTime() then
			    Note_Set(Check_UI("等待技能... ","Wait Spell... ") .. rs["寒冰屏障"])
				return
			end
			HasStop = false
			Dungeon_step1 = Dungeon_step1 + 1
	...
	if Dungeon_step1 == 5 then -- 开始流程
	    Note_Head = Check_UI("拉怪","Pulling mobs")

		local Path = 
		{
		{870.94,1348.80,18.01},
		{870.61,1368.89,18.01}, -- 2 奥爆
		{874.70,1398.78,18.01},
		{880.17,1401.83,18.67}, -- 4 奥爆
		{887.39,1401.95,18.67},
		{895.62,1411.71,18.67}, -- 6 奥爆
		{906.99,1416.40,18.01}, -- 7 奥爆
		{927.04,1422.22,18.02}, -- 8 奥爆
		{952.44,1421.85,18.02}, -- 9 奥爆
		{962.18,1424.00,18.67}, -- 10 奥爆
		{962.64,1435.10,18.67}, -- 11 奥爆
		{923.61,1435.36,18.67}, -- 12 闪现
		{912.90,1435.34,18.67}, -- 13 冰环
		{892.29,1434.18,18.67}, -- 14 奥爆
		{893.16,1375.99,18.67},
		{892.97,1363.02,18.67}, -- 16 奥爆
		{932.58,1362.67,18.67}, -- 17 闪现
		{962.79,1363.82,18.67}, -- 18 奥爆
		{965.98,1377.57,18.67}, -- 19 冰环
		{983.54,1377.52,24.29},
		{990.44,1345.38,29.14}, -- 21 奥爆
		{1042.63,1346.80,29.14}, -- 22 奥爆
		{1047.26,1365.58,27.29}, -- 23 奥爆
		{1047.40,1393.97,27.30}, -- 24 闪现 反制 1064.39,1387.32,30.76
		{1048.35,1410.70,27.30}, -- 25 火冲 1065,1410,30
		{1048.23,1420.28,27.29},
		{1045.31,1448.03,29.14}, -- 27 冰环
		{1041.84,1452.68,29.14}, -- 28 奥爆
		{988.66,1451.52,29.14},
		{983.42,1419.42,24.29}, -- 30 闪现
		{971.01,1421.25,20.12}, -- 31 冰箱
		}
		...
		if Distance > 0.9 then
			...
			if Dungeon_move == 12 or Dungeon_move == 17 or Dungeon_move == 24 or Dungeon_move == 30 then -- 闪现
			    ...
			    if Distance >= 7 then
					CheckProtection()
					if not CheckBuff("player",rs["防护冰霜结界"]) and Spell_Castable(rs["防护冰霜结界"]) then
					    awm.CastSpellByName(rs["防护冰霜结界"],"player")
					end
				end
			end

			if Dungeon_move == 4 and not HasStop then
			    HasStop = true
			...
			if Dungeon_move == 29 then
			    local starttime, duration, enabled, _ = GetSpellCooldown(rs["寒冰屏障"])
				local endtime = starttime + duration
				if endtime > GetTime() and Spell_Castable(rs["急速冷却"]) then
		...
		elseif Distance <= 0.9 then
			...
			if Dungeon_move == 2 or Dungeon_move == 4 or (Dungeon_move >= 6 and Dungeon_move <= 11) or Dungeon_move == 14 or Dungeon_move == 16 or Dungeon_move == 18 or Dungeon_move == 21 or Dungeon_move == 22 or Dungeon_move == 23 or Dungeon_move == 28 then
			...
			if Dungeon_move == 13 or Dungeon_move == 19 or Dungeon_move == 27 then
			    awm.CastSpellByName(rs["冰霜新星"])
			end

			if Dungeon_move == 24 then -- 反制
				local Scan_Distance = 8
				if Dungeon_move == 24 then
				    Target_ID = nil
					tarx,tary,tarz = 1064.39,1387.32,30.76
					Scan_Distance = 5
				end
			...
			if Dungeon_move == 25 then -- 火冲
				local Scan_Distance = 5
				if Dungeon_move == 25 then
				    Target_ID = nil
					tarx,tary,tarz = 1065,1410,30
					Scan_Distance = 7
				end
				if Target_Monster == nil then
				    Target_Monster = Find_Object_Position(Target_ID,tarx,tary,tarz,Scan_Distance)
			...
			if Dungeon_move == 31 then
				...
				for i = 1,total do
					...
					local target = awm.UnitTarget(ThisUnit)
					local distance = awm.GetDistanceBetweenObjects("player",ThisUnit)
					if awm.UnitAffectingCombat(ThisUnit) and not awm.UnitIsDead(ThisUnit) and awm.UnitCanAttack("player",ThisUnit) and distance <= 8 then
						awm.CastSpellByName(rs["寒冰屏障"])
					end
				end
				return
	...
	if Dungeon_step1 == 6 then -- 出冰箱
	    local x,y,z = 939.58,1419.41,18.02
	    local distance = awm.GetDistanceBetweenPositions(Px,Py,Pz,x,y,z)
		if distance >= 0.9 then
		    awm.MoveTo(x,y,z)
	...
	if Dungeon_step1 == 7 then -- A怪
		Note_Head = Check_UI("击杀","AOE killing")

	    local Path = 
		{
		{939.58,1414.71,19.73}, -- 下雪 958.39,1410.87,18.89
		{939.58,1419.41,18.02}, -- 下雪 960.43,1417.85,18.35
		}
		local Coord = Path[Dungeon_move]
		local x,y,z = Coord[1],Coord[2],Coord[3]
		local Distance = awm.GetDistanceBetweenPositions(Px,Py,Pz,x,y,Pz)
		...
		if Distance > 0.9 then
			...
			for i = 1,#Monster do
				...
				local distance = awm.GetDistanceBetweenPositions(Px,Py,Pz,tarx,tary,tarz)
				if distance <= 8 then
					if Spell_Castable(rs["魔爆术"]) and (UnitPower("player") >= 1000 or CheckBuff("player",rs["节能施法"])) then
						awm.CastSpellByName(rs["魔爆术"])
					end
				end
			end
		...
		elseif Distance <= 0.9 then 
			...
			if Dungeon_move == 2 then -- 暴风雪
			    First_Blizzard = true
			    Note_Set(Dungeon_move.." | "..#Monster)
			    local sx,sy,sz = 960.43,1417.85,18.35
				local s_time = 6
				local Moshu = false
				local Run_Away = false -- 所有怪物都跑了
				local target = nil
				...
				for i = 1,#Monster do
					local ThisUnit = Monster[i]
					local tarx,tary,tarz = awm.ObjectPosition(ThisUnit)
					local distance = awm.GetDistanceBetweenPositions(950.45,1417.80,18.02,tarx,tary,tarz)
					if tarz <= 18.2 and distance <= 2 and awm.UnitTarget(ThisUnit) then
						XS_Timer = false
						log_Spell = false
						awm.SpellStopCasting()
						if Spell_Castable(rs["寒冰护体"]) and not CheckBuff("player",rs["寒冰护体"]) then
					...
					if awm.ObjectId(ThisUnit) ~= 4294 then
					    Moshu = true
					end
					if awm.GetDistanceBetweenPositions(sx,sy,sz,tarx,tary,tarz) <= 20 and awm.ObjectId(ThisUnit) ~= 4294 then
					    Run_Away = true
					end
					if awm.GetDistanceBetweenPositions(Px,Py,Pz,tarx,tary,tarz) < Far_Distance and awm.ObjectId(ThisUnit) ~= 4294 then
					    target = ThisUnit
						Far_Distance = awm.GetDistanceBetweenPositions(Px,Py,Pz,tarx,tary,tarz)
					end
				end

				if not Moshu then
				    textout(Check_UI("怪物清空, 奥爆魔术师","All swing mobs dead.."))
				    Dungeon_step1 = 8
					return
				end
				...
				if not Run_Away then
				    s_time = 3
					if not XS_Timer and awm.GetDistanceBetweenObjects("player",target) <= 8 then
						XS_Timer = true
						XS_Time = GetTime()
					end
				end
				...
				if not Run_Away then
				    if awm.GetDistanceBetweenObjects("player",target) <= 8 then
						if Spell_Castable(rs["魔爆术"]) and (UnitPower("player") >= 1000 or CheckBuff("player",rs["节能施法"])) then
							awm.CastSpellByName(rs["魔爆术"])
						else
							awm.CastSpellByName(rs["魔爆术(等级 1)"])
				...
				if not CastingBarFrame:IsVisible() then
				    if not awm.IsAoEPending() then
						...
						elseif awm.UnitPower("player") > 1300 then
							awm.CastSpellByName(rs["暴风雪"])
							if not log_Spell then
								log_Spell = true
			...
			if Dungeon_move == 1 then -- 暴风雪
			    Note_Set(Dungeon_move.." | "..#Monster)
			    local sx,sy,sz = 958.39,1410.87,18.89
				local s_time = 5
				if not First_Blizzard then
				    s_time = 7.5
				end
				local Moshu = false
				local Run_Away = false -- 所有怪物都跑了
				...
				for i = 1,#Monster do
					local ThisUnit = Monster[i]
					local tarx,tary,tarz = awm.ObjectPosition(ThisUnit)
					local distance = awm.GetDistanceBetweenPositions(950.43,1415.14,19.73,tarx,tary,tarz)
					if tarz >= 18.3 and distance <= 2 and awm.UnitTarget(ThisUnit) then
						XS_Timer = false
						log_Spell = false
						awm.SpellStopCasting()
						if Spell_Castable(rs["寒冰护体"]) and not CheckBuff("player",rs["寒冰护体"]) then
	...
	if Dungeon_step1 == 8 then -- 魔术师
		...
		if not CastingBarFrame:IsVisible() then
			...
			elseif distance <= 8 then
			    if UnitPower("player") >= 1000 or CheckBuff("player",rs["节能施法"]) then
			        awm.CastSpellByName(rs["魔爆术"])
				else
				    awm.CastSpellByName(rs["魔爆术(等级 1)"])
				end
...
function MainThread()
	...
	if (Easy_Data["需要卖物"] or Easy_Data["需要修理"]) then
	    if not Check_BagFree() or Sell.Step ~= 1 then
			...
			if Instance == 189 and Dungeon_step == 2 then
		    ...
		    elseif Instance ~= 189 then
				...
				if GetItemCount(6948) > 0 and durationtime < 10 then
				    ...
				    if awm.GetDistanceBetweenPositions(Px,Py,Pz,2910,-818.83,160.29) <= 30 then
