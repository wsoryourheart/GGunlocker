...
function Death_Run() -- 角色死亡，用寻路call跑尸体
	...
	local distance = awm.GetDistanceBetweenPositions(2915,-823,160,Px,Py,Pz)
	local distance1 = awm.GetDistanceBetweenPositions(2884,-836,160,Px,Py,Pz)
	if distance < 10 then
	    if not SP_Timer then
		    ...
		    if GetTime() - SP_Time < 10 then
			    return
			end
		end

	    local x,y,z = 2925,-822,160.32
		Run(x,y,z)
		return
	elseif distance1 < 10 then
	    if not SP_Timer then
		    SP_Timer = true
			SP_Time = GetTime()
			return
...
local function Create_Config_UI() -- 游戏设置
	...
	local function Chat_UI() -- 喊话命令
		Basic_UI.Set.Py = Basic_UI.Set.Py - 30
		local Header2 = Create_Header(Basic_UI.Set.frame,"TOPLEFT",10, Basic_UI.Set.Py,Check_UI("跟随命令","Follow Command")) 

		Basic_UI.Set.Py = Basic_UI.Set.Py - 20
		Basic_UI.Set["跟随命令"] = Create_EditBox(Basic_UI.Set.frame,"TOPLEFT",10, Basic_UI.Set.Py,"11",false,280,24)

		Basic_UI.Set["跟随命令"]:SetScript("OnEditFocusLost", function(self)
			Easy_Data["跟随命令"] = Basic_UI.Set["跟随命令"]:GetText()
		end)
		if Easy_Data["跟随命令"] ~= nil then
			Basic_UI.Set["跟随命令"]:SetText(Easy_Data["跟随命令"])
		else
			Easy_Data["跟随命令"] = Basic_UI.Set["跟随命令"]:GetText()
		end

		Basic_UI.Set.Py = Basic_UI.Set.Py - 30
		local Header2 = Create_Header(Basic_UI.Set.frame,"TOPLEFT",10, Basic_UI.Set.Py,Check_UI("停止跟随命令","Stop Follow Command")) 

		Basic_UI.Set.Py = Basic_UI.Set.Py - 20
		Basic_UI.Set["停止跟随命令"] = Create_EditBox(Basic_UI.Set.frame,"TOPLEFT",10, Basic_UI.Set.Py,"22",false,280,24)

		Basic_UI.Set["停止跟随命令"]:SetScript("OnEditFocusLost", function(self)
			Easy_Data["停止跟随命令"] = Basic_UI.Set["停止跟随命令"]:GetText()
		end)
		if Easy_Data["停止跟随命令"] ~= nil then
			Basic_UI.Set["停止跟随命令"]:SetText(Easy_Data["停止跟随命令"])
		else
			Easy_Data["停止跟随命令"] = Basic_UI.Set["停止跟随命令"]:GetText()
		end
	end
...
function Script:BeginEvent(event,arg1,arg2,_,_,arg5,arg6,_,_,_,_,_,_,_)
	...
	if event == "CHAT_MSG_PARTY_LEADER" then
	    if arg1 == Easy_Data["跟随命令"] then
		    awm.TargetUnit(arg5)
			awm.FollowUnit = awm.Unlock("FollowUnit")
			awm.FollowUnit("target")
		elseif arg1 == Easy_Data["停止跟随命令"] then
		    local x,y,z = awm.ObjectPosition("player")
			awm.Interval_Move(x,y,z)
		end
	end
