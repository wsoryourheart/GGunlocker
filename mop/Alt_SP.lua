...
function Go_Out_Dungeon()
	...
	local Path = 
	{
	{677.19,6859.68,-72.13},{668.91,6862.13,-75.58},{658.28,6865.26,-81.57},{652.42,6866.18,-82.56},{640.99,6865.70,-79.47},{631.53,6874.67,-74.60},{624.14,6881.87,-69.89},{615.56,6888.97,-57.20},{610.75,6892.74,-49.10},{607.54,6900.28,-48.22},{603.49,6909.79,-47.12},{598.56,6918.92,-45.57},{585.83,6932.56,-42.02},{577.34,6939.47,-40.89},{568.93,6940.79,-26.77},{564.99,6941.14,-5.38},{565.56,6940.61,16.84}
	}
	if not Using_Fixed_Path then
	    Using_Fixed_Path = true
		Fixed_Finish = false
...
function MainThread()
	...
	if Instance == 547 then
		...
		if Dungeon_step == 1 then
		    ...
		    local distance = awm.GetDistanceBetweenPositions(Px,Py,Pz,109.52,-128.50,2.42)
			if distance >= 1 then
			    Note_Set(math.floor(distance))
			    Run(109.52,-128.50,2.42)
			else
			    local leader = nil
		...
		if distance > 1 and not Dungeon_Flush then
			...
			if (GetSubZoneText() ~= Shui_Ku and not Using_Fixed_Path and not Fixed_Finish) or (Using_Fixed_Path and not Fixed_Finish) then
				Go_In_Dungeon()
				return
			end
