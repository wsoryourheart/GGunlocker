...
textout(Check_UI("野外剥皮练习 - "..Function_Version,"Skin Practice - "..Function_Version))
...
function Skin_Find()
    ...
    for i = 1, GetNumSkillLines() do 
		...
		if string.find (skillName, rs["剥皮"]) then
		    Skin_Rank = skillRank
		end
	end
	if Skin_Rank < 75 then
		Find_Object(113,Find_List)
		Find_Object(822,Find_List)
		Find_Object(524,Find_List)
	end
	if Skin_Rank <= 120 and Skin_Rank >= 75 then
		Find_Object(547,Find_List)
		Find_Object(157,Find_List)
	end

	if Skin_Rank <= 155 and Skin_Rank >= 100 then
		Find_Object(213,Find_List)
		Find_Object(565,Find_List)
	end

	if Skin_Rank <= 175 and Skin_Rank >= 155 then
		Find_Object(681,Find_List)
		Find_Object(683,Find_List)
		Find_Object(1150,Find_List)
	end

	if Skin_Rank <= 215 and Skin_Rank >= 175 then
		Find_Object(686,Find_List)
		Find_Object(1085,Find_List)
		Find_Object(681,Find_List)
		Find_Object(1150,Find_List)
		Find_Object(1108,Find_List)
	end

	if Skin_Rank <= 255 and Skin_Rank >= 215 then
		Find_Object(690,Find_List)
		Find_Object(687,Find_List)
	end

	if Skin_Rank <= 300 and Skin_Rank >= 255 then
		Find_Object(2521,Find_List)
		Find_Object(2522,Find_List)
	end

	if Skin_Rank >= 300 then
		Find_Object(16879,Find_List)
	end
	
	if Skin_Rank <= 75 and Skin_Rank >= 0 then
		Find_Object(1553,Find_List)
		Find_Object(1554,Find_List)
		Find_Object(1548,Find_List)
	end

	if Skin_Rank <= 120 and Skin_Rank >= 75 then
		Find_Object(1924,Find_List)
		Find_Object(1923,Find_List)
	end

	if Skin_Rank <= 155 and Skin_Rank >= 120 then
		Find_Object(2350,Find_List)
		Find_Object(2351,Find_List)
	end
...
function Find_Corpse()
	...
	for i = 1,total do
		...
		if awm.IsGuid(ThisUnit) then
			...
			if (awm.UnitIsLootable(guid) or awm.UnitIsSkinnable(ThisUnit)) and awm.UnitIsDead(ThisUnit) and awm.ObjectIsUnit(ThisUnit) then
...
function Path_Information()
	...
	if Faction == "Horde" then
	    if SR >= 0 and SR < 75 then
		    Mobs_MapID = 1420
		    Mobs_Coord = {{2023.26,362.73,43.18},{2008.79,451.86,38.40},{2001.62,567.73,40.27}}
		...
		if SR >= 75 and SR < 120 then
		    Mobs_MapID = 1421
		    Mobs_Coord = {{-481.51,1173.75,63.55},{-463.45,1229.34,51.04},{-438.51,1296.85,45.23},{-476.35,1373.69,34.76},{-552.96,1422.15,19.81}}
		...
		if SR >= 120 and SR < 155 then
		    Mobs_MapID = 1424
		    Mobs_Coord = {{-513.62,308.68,86.00},{-532.39,252.86,73.19},{-447.39,222.32,80.22},{-349.86,194.75,86.90},{-278.78,186.56,91.34},{-231.59,130.23,68.00},{-203.48,97.05,63.95}}
		...
		if SR >= 155 and SR < 175 then
		    
			Mobs_MapID = 1434
		    Mobs_Coord = {{ -11564.2890625, 170.98567199707, 17.209957122803 },
		...
		if SR >= 215 and SR < 255 then
		    
			Mobs_MapID = 1434
		    Mobs_Coord = {{ -12983.04, 374.3563, 22.08989 },
				  { -12849.72, 413.3459, 13.65897 },
				  { -12769.1, 289.7581, 9.972009 },
				  { -12817.78, 126.0833, 18.85758 },
				  { -12888.9, 16.74098, 19.13797 },
				  { -12952.55, -107.0867, 10.70257 }}
		...
		if SR >= 255 and SR < 300 then
		    
			Mobs_MapID = 1434
		    Mobs_Coord = {{ -14747.76, -446.286, 2.74223 },
				  { -14813.71, -398.3734, 1.874197 },
				  { -14845.42, -318.0526, 4.246517 },
				  { -14836.09, -254.6308, 5.971158 },
				  { -14800.74, -168.1333, 4.010108 },
				  { -14716.7, -166.2746, 8.100977 },
				  { -14625.35, -152.3842, 4.937718 },
				  { -14560.12, -227.8214, 10.57597 },
				  { -14537.37, -285.1754, 12.44321 },
				  { -14563.8, -376.9256, 5.845669 },
				  { -14510.45, -440.5727, 2.612716 }}
		...
		if SR >= 300 and SR <= 375 then
		    
			Mobs_MapID = 1944
		    Mobs_Coord = {{-1016.08,2424.03,11.16},{-921.98,2336.54,-5.15},{-824.54,2212.60,7.23},{-751.06,2207.63,13.05},{-624.35,2208.46,48.00},{-649.94,2098.04,50.35},{-710.60,2004.60,45.81}}
	    ...
	    if SR >= 0 and SR < 75 then
			...
			Mobs_MapID = 1429
		    Mobs_Coord = {{ -9684.514, 104.4446, 45.87589 },
				  { -9798.877, 65.72743, 39.22227 },
				  { -9825.307, 24.2936, 30.51624 },
				  { -9942.538, 98.83311, 32.88113 },
				  { -10008.73, 192.8474, 30.83642 },
				  { -9934.201, -70.11047, 32.25984 },
				  { -9794.248, -66.07594, 26.09071 },
				  { -9745.31, 44.77966, 38.91861 }}
		...
		if SR >= 75 and SR <= 120 then
			...
			Mobs_MapID = 1436
		    Mobs_Coord = {{ -10580.4, 906.7961, 39.03373 },
				  { -10460.39, 911.8939, 38.99928 },
				  { -10398.29, 911.915, 44.54536 },
				  { -10474.49, 926.1497, 38.2642 },
				  { -10688.25, 774.1604, 48.62919 },
				  { -11019.71, 674.3362, 36.10422 },
				  { -10966.29, 900.0272, 35.76817 },
				  { -10962.93, 1041.176, 39.14428 },
				  { -10989.44, 1162.191, 41.0705 },
				  { -10793, 1211.642, 40.48429 },
				  { -10710.73, 961.4429, 43.50329 }}
		...
		if SR > 120 and SR < 155 then
			...
			Mobs_MapID = 1431
		    Mobs_Coord = {{ -10170.1, 7.936629, 25.66436 },
				  { -10219.46, -45.26642, 32.47087 },
				  { -10199.19, -114.5439, 30.1183 },
				  { -10131.19, -185.7137, 29.95372 },
				  { -10072.91, -233.3735, 31.83663 },
				  { -10062.18, -326.7123, 29.42694 },
				  { -10051.83, -656.7457, 41.43805 },
				  { -10069.96, -820.1758, 36.38235 },
				  { -10040.21, -335.5433, 28.08456 },
				  { -10068.34, -184.4931, 28.11967 },
				  { -10115.9, -112.7761, 26.77581 },
				  { -10146.74, -39.95595, 27.16183 },
				  { -10192.11, 85.43484, 26.64569 },
				  { -10207.65, 220.3788, 21.75087 },
				  { -10229.15, 470.5668, 33.35358 }}
		...
		if SR >= 175 and SR < 215 then
		    ...
		    Mobs_Coord = {{ -11564.2890625, 170.98567199707, 17.209957122803 },
					...
					{ -11622.171875, -382.38363647461, 16.777088165283 },
					{ -11687.310546875, -482.69644165039, 16.000625610352 },
					{-11936.36,47.18,16.53},
					{-11970.24,-66.97,4.11},
					{-11927.50,-137.88,12.37},
					}
...
function Skin_Process()
	...
	if Grind.Step == 2 then -- 采集
		...
		if not awm.ObjectExists(Target_Info.Item) and distance < 80 then
		    Coordinates_Get = false
			Mount_useble = GetTime()
			Tried_Mount = GetTime()
			...
			return
		elseif not awm.UnitIsLootable(Target_Info.Item) and (not awm.UnitIsSkinnable(Target_Info.Item) or (awm.UnitLevel(Target_Info.Item) * 5 > Skill_Level(rs["剥皮"]) and Skill_Level(rs["剥皮"]) > 100)) and awm.UnitIsDead(Target_Info.Item) then
...
function MainThread()
	...
	local Skill_level,Max_Level = Skill_Level(rs["剥皮"])
	if not DoesSpellExist(rs["剥皮"]) or (Skill_level == Max_Level and Max_Level ~= 375) then
		if Skill_level < 300 then
			if Faction == "Horde" then
				Trainer_Coord.mapid,Trainer_Coord.x,Trainer_Coord.y,Trainer_Coord.z = 1458,1499.18,200.12,-62.13
				Trainer_Name = Check_Client("基里安·哈根","Killian Hagey")
			else
				Trainer_Coord.mapid,Trainer_Coord.x,Trainer_Coord.y,Trainer_Coord.z = 1453,-8716.47,469.25,98.61
				Trainer_Name = Check_Client("马瑞斯·格兰治","Maris Granger")
			end
		else
			if Faction == "Horde" then
				Trainer_Coord.mapid,Trainer_Coord.x,Trainer_Coord.y,Trainer_Coord.z = 1944,151.23,2633.31,86.54
				Trainer_Name = Check_Client("穆鲁图","Moorutu")
			else
				Trainer_Coord.mapid,Trainer_Coord.x,Trainer_Coord.y,Trainer_Coord.z = 1944,-693.83,2724.69,94.11
				Trainer_Name = Check_Client("耶蕾娜·夜空","Jelena Nightsky")
			end
		end
			
		Note_Head = Check_UI("学习剥皮","Learn Skining")
...
local function Create_GUIDE_UI() -- 功能说明UI
	...
	local Header = Create_Header(Basic_UI.Guide.frame,"TOPLEFT",10, Basic_UI.Guide.Py,Check_UI("1 - 375 剥皮 野外练习","1 - 375 Skining Level up")) 
	...
	Basic_UI.Guide.Py = Basic_UI.Guide.Py - 30
	local Header = Create_Header(Basic_UI.Guide.frame,"TOPLEFT",10, Basic_UI.Guide.Py,Check_UI("请自行购买 - ","Please buy yourself with - ")..Check_Client("剥皮小刀","Skinning Knife"))

	Basic_UI.Guide.Py = Basic_UI.Guide.Py - 30
	local Header = Create_Header(Basic_UI.Guide.frame,"TOPLEFT",10, Basic_UI.Guide.Py,Check_UI("1 - 375 自动学习剥皮技能","1 - 375 Auto learn skin skill"))
	...
	Basic_UI.Guide.Py = Basic_UI.Guide.Py - 30
	local Header = Create_Header(Basic_UI.Guide.frame,"TOPLEFT",10, Basic_UI.Guide.Py,Check_UI("1 - 75 部落 提瑞斯法林地","1 - 75 Horde Tirisfal Glades"))

	Basic_UI.Guide.Py = Basic_UI.Guide.Py - 30
	local Header = Create_Header(Basic_UI.Guide.frame,"TOPLEFT",10, Basic_UI.Guide.Py,Check_UI("1 - 75 联盟 艾尔文森林","1 - 75 Alliance Elwynn Forest"))

	Basic_UI.Guide.Py = Basic_UI.Guide.Py - 30
	local Header = Create_Header(Basic_UI.Guide.frame,"TOPLEFT",10, Basic_UI.Guide.Py,Check_UI("75 - 120 部落 银松森林","75 - 120 Horde Silverpine Forest"))

	Basic_UI.Guide.Py = Basic_UI.Guide.Py - 30
	local Header = Create_Header(Basic_UI.Guide.frame,"TOPLEFT",10, Basic_UI.Guide.Py,Check_UI("75 - 120 联盟 西部荒野","75 - 100 Westfall"))

	Basic_UI.Guide.Py = Basic_UI.Guide.Py - 30
	local Header = Create_Header(Basic_UI.Guide.frame,"TOPLEFT",10, Basic_UI.Guide.Py,Check_UI("120 - 155 部落 希尔斯布莱德丘陵","120 - 155 Horde Hillsbrad Foothills"))

	Basic_UI.Guide.Py = Basic_UI.Guide.Py - 30
	local Header = Create_Header(Basic_UI.Guide.frame,"TOPLEFT",10, Basic_UI.Guide.Py,Check_UI("120 - 155 联盟 暮色森林","100 - 155 Alliance Duskwood"))

	Basic_UI.Guide.Py = Basic_UI.Guide.Py - 30
	local Header = Create_Header(Basic_UI.Guide.frame,"TOPLEFT",10, Basic_UI.Guide.Py,Check_UI("155 - 300 荆棘谷","155 - 300 Stranglethorn Vale"))

	Basic_UI.Guide.Py = Basic_UI.Guide.Py - 30
	local Header = Create_Header(Basic_UI.Guide.frame,"TOPLEFT",10, Basic_UI.Guide.Py,Check_UI("300 - 375 外域","300 - 375 OutLand"))
end
...
local Initial_Skin_Level = Skill_Level(rs["剥皮"])
...
Detail_Frame:SetScript("OnUpdate", function()
    if not Generate then
		...
		Mine_Level_Monitor:SetText(Check_UI("采药等级提升: ","Herbalism Level up: ")..Initial_Skin_Level)
		...
		Mine_Level_Monitor:SetText(Check_UI("剥皮等级提升: ","Skining Level up: ")..(Skill_Level(rs["剥皮"]) - Initial_Skin_Level))
		Loot_Amount_Monitor:SetText(Check_UI("已击杀怪物: ","Killed total amount: ")..#Monster_Has_Killed)
		Black_List_Monitor:SetText(Check_UI("已拉黑怪物: ","Blacklist total amount: ")..#Black_List)
	end
end)