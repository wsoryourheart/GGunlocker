...
local Dungeon_In = {mapid = 1453, x = -8750, y = 850, z = 70}
...
function MainThread()
	...
	if Instance == 34 then
		...
		if Dungeon_step == 1 then
		    ...
		    local distance = awm.GetDistanceBetweenPositions(Px,Py,Pz,53.32,0.29,-18.00)
			if distance >= 1 then
			    Note_Set(math.floor(distance))
			    Run(53.32,0.29,-18.00)
			else
			    local leader = nil
				...
				if leader ~= nil then
				    Note_Set(Check_UI("队长 = ","Leader = ")..leader)
